/**
 * enlace — FIXED (aligned to repo gateway worker)
...
// -------------------------
// Security headers (OPS CyberSec Core: OWASP/NIST/CISA/PCI + GSC-ready)
// -------------------------
...
function stripDangerousMarkup(text) {
  ...
  t = t.replace(
    /<\s*\/\s*(iframe|object|embed|link|meta|base|form)\s*>/gi,
    ""
  );
...
function userExplicitlyAskedAuthor(messages) {
  ...
  const namePlusQuestion =
    /\bgabriel\s+anangono\b/.test(q) &&
    /\b(who|quien|quién|autor|author|creator|creador|created|built|hizo|creó|creo)\b/.test(q);
...
function stripAuthorIfNotAsked(text, allow) {
  ...
  t = t.replace(/\bAutor:\s*Gabriel\s+Anangono\.?\b/gi, "");
...
async function detectLangIso2ViaModel(env, text) {
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "Return ONLY the ISO 639-1 language code (two letters). If unsure, return 'und'. No extra text.",
        },
    ...
    const raw = String(
      out?.response || out?.result?.response || out?.text || out || ""
    )
      .trim()
      .toLowerCase();
...
async function detectLangIso2(env, messages, metaSafe) {
  ...
  if (metaLang && metaLang !== "und" && metaLang !== "auto") {
    if (heur && heur !== metaLang) return heur;
    return metaLang;
  }
...
function normalizeAllowedChatModel(maybe) {
  const s = safeTextOnly(maybe || "");
  if (!s) return "";
  if (s === MODEL_CHAT_FAST) return MODEL_CHAT_FAST;
  if (s === MODEL_CHAT_FALLBACK) return MODEL_CHAT_FALLBACK;
  if (s === MODEL_CHAT_SPANISH_QUALITY) return MODEL_CHAT_SPANISH_QUALITY;
  return "";
}
...
function sanitizeMeta(metaIn) {
  ...
  const sq = safeTextOnly(meta.spanish_quality || "").toLowerCase();
  if (sq) out.spanish_quality = sq;

  // explicit model override (whitelisted only)
  const explicitModel = normalizeAllowedChatModel(meta.model || "");
  if (explicitModel) out.model = explicitModel;

  const tone = safeTextOnly(meta.tone || "").toLowerCase();
  if (tone) out.tone = tone === "friendly" || tone === "strict" ? tone : "professional";

  const rf = safeTextOnly(meta.reply_format || "").toLowerCase();
  if (rf) out.reply_format = rf === "list" ? "list" : "paragraph";

  const mt = safeTextOnly(meta.model_tier || "").toLowerCase();
  if (mt === "fast" || mt === "quality" || mt === "fallback") out.model_tier = mt;
...
function parseSSEBlockToData(block) {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("event:")) {
      evt = line.slice(6); // keep possible leading spaces
      continue;
    }
    if (line.startsWith("data:")) {
      dataLines.push(line.slice(5)); // DO NOT trim payload
      continue;
    }
  }
...
function getDeltaFromObj(obj) {
  ...
  if (obj.response && obj.response.response && typeof obj.response.response === "string")
        ...
        while (true) {
          ...
          if (looksLikeSSE) {
            ...
            for (const block of blocks) {
              ...
              // Try JSON parse if it *looks* like JSON (using trimmed copy only)
              const d0 = dataTrim[0];
              if (d0 === "{" || d0 === "[") {
                try {
                  ...
                  const safeData = stripAuthorIfNotAsked(String(data), allowAuthor);
  ...
  async fetch(request, env) {
    ...
    if (isChat) {
      ...
      if (!ct.includes("application/json"))
        return json(415, { error: "content-type must be application/json" }, baseExtra);

      const raw = await request.text().catch(() => "");
      if (!raw || raw.length > MAX_BODY_CHARS)
        return json(413, { error: "Request too large" }, baseExtra);
      ...
      if (ct.includes("application/json")) {
        ...
        if (typeof body.audio_b64 === "string" && body.audio_b64.length) {
          ...
          if (bytes.byteLength > MAX_AUDIO_BYTES)
            return json(413, { error: "Audio too large" }, baseExtra);
          audioU8 = bytes;
